...
class StddevGraphDx():
    ...
    def __init__(self, pos, relative_to=None, dock_name=''):
        ...
        self.graphs[self.__id]['widget'].setLabel('bottom', 'distance', units='osu!px', unitPrefix='')
    ...
    def plot_data(self, data):
        ...
        for bpm in unique_bpms:
            ...
            pxs = data[bpm_filter, self.COL_PX]

            idx_sort = np.argsort(pxs)

            color = bpm_lut.map(bpm, 'qcolor')
            self.graphs[self.__id]['widget'].plot(x=pxs[idx_sort], y=stddevs[idx_sort]**2, symbol=symbol, symbolPen='w', symbolSize=10, pen=color, symbolBrush=color, name=f'{bpm} bpm')
